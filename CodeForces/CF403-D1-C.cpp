...
Solution - If we interpret the matrix as 0, 1 (non-zero values are 1) and replace addition with OR,
multiplication with AND, what we want is that the transitive closure of this matrix is all 1's. In
other words, if we interpret a_ij = 1 as an edge from i -> j, we want this graph to have paths b/w
all pairs of vertices, which we can check using SCC.
*/
